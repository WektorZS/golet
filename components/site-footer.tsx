  ...
  return (
    <footer className="bg-foreground text-background">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 md:grid-cols-4 md:px-6">
        ...
        <div className="flex flex-col gap-3 text-sm"><h2 className="font-mono text-xs font-bold uppercase tracking-widest text-primary">Dane firmy</h2><p>{content.companyName || "Let's Gol Sp. z o.o."}</p><p>{content.companyAddress || "00-100 Warszawa"}</p><p>NIP: {content.companyNip || "123 456 78 90"}</p><p>REGON: 520474445</p></div>
      ...
      <div className="border-t border-background/10"><div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-5 text-xs text-background/50 md:flex-row md:items-center md:justify-between md:px-6"><p>© 2026 Let&apos;s Gol. Wszystkie prawa zastrzeżone.</p><div className="flex flex-wrap gap-4"><Link href="/informacje-prawne">Polityka prywatności</Link><Link href="/warunki-uczestnictwa">Warunki uczestnictwa</Link></div></div></div>
    </footer>
  )
}
