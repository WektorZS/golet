...
export function HomeTripCalendar({ trips }: { trips: Trip[] }) {
  ...
  return (
    <div className="mt-10">
      ...
      <div role="tablist" aria-label="Miesiąc wyjazdu" className="flex snap-x gap-2 overflow-x-auto border-b pb-3 pr-14 scrollbar-thin">
        {months.map((month) => {
          ...
          return (
            <button
              ...
              className={`flex min-w-36.25 shrink-0 snap-start items-center justify-between gap-2 rounded-lg px-4 py-2.5 font-mono text-xs font-black tracking-wide transition-colors sm:min-w-0 ${
                active
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 right-0 flex w-14 items-center justify-end bg-linear-to-l from-background via-background/90 to-transparent pr-1 sm:hidden"><ArrowRight className="size-4 text-primary" /></div>
      </div>
