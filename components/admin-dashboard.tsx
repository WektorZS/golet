...
function GalleryDialog({ asset, trips }: { asset: any; trips: any[] }) {
  ...
  return <Dialog><DialogTrigger render={<Button variant="outline" size="sm" />}>Użyj</DialogTrigger><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Użyj zdjęcia</DialogTitle><DialogDescription>Dodaj zdjęcie do galerii albo ustaw je jako okładkę wyjazdu.</DialogDescription></DialogHeader><form action={action} className="flex flex-col gap-4"><input type="hidden" name="mediaId" value={asset.id} /><Field label="Miejsce w galerii"><select name="tripId" className="h-9 rounded-lg border bg-background px-3"><option value="0">Galeria strony głównej</option>{trips.map((trip) => <option key={trip.id} value={trip.id}>{trip.title}</option>)}</select></Field><Field label="Podpis"><Input name="caption" /></Field><Field label="Miasto (galeria główna)"><Input name="city" /></Field><Field label="Alt"><Input name="alt" defaultValue={asset.alt} /></Field>{state.error && <p role="alert" className="text-sm text-destructive">{state.error}</p>}{state.success && <p role="status" className="text-sm font-medium text-primary">{state.message}</p>}<Button type="submit" disabled={pending}>{pending ? "Dodaję…" : "Dodaj do galerii"}</Button></form><div className="border-t pt-5"><form action={setTripCover} className="flex flex-col gap-4"><input type="hidden" name="mediaId" value={asset.id} /><Field label="Ustaw jako zdjęcie główne"><select name="tripId" className="h-9 rounded-lg border bg-background px-3" required><option value="">Wybierz wyjazd</option>{trips.map((trip) => <option key={trip.id} value={trip.id}>{trip.title}</option>)}</select></Field><DialogFooter><Button variant="outline">Ustaw okładkę</Button></DialogFooter></form></div></DialogContent></Dialog>
}
