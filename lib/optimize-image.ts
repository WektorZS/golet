...
export async function optimizeUploadedImage(file: File) {
  if (file.size === 0) throw new Error("Wybierz plik")
  if (file.size > MAX_INPUT_SIZE) throw new Error("Zdjęcie może mieć maksymalnie 15 MB")
  ...
  if (!metadata.width || !metadata.height) throw new Error("Nie udało się odczytać wymiarów zdjęcia")
