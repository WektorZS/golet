...
async function resolveChannelId(channelUrl: string) {
  ...
  try {
    const response = await fetch(url.toString(), { cache: "no-store", headers: { "User-Agent": "Mozilla/5.0" } })
...
export async function syncYouTubeVideos() {
  ...
  const response = await fetch(`https://www.youtube.com/feeds/videos.xml?channel_id=${channelId}`, { cache: "no-store" })
  if (!response.ok) {
    await recordSyncResult(`Błąd: YouTube odpowiedział kodem ${response.status}`)
